...
def ler_xml_nfe(nota):
    ...
    if 'nfeProc' in documento:
        ...
        if local_op == '1':
            ...
            operacao = 'Operação com Exterior'
        if 'eveNFe' in documento['nfeProc']:
            try:
                dic_evento = documento['NFeLog']['eveNFe']
                carta_correcao_r = 'Não houve'
                cancelamento_r = 'Não houve'
...
def ler_xml_cte(nota):
    ...
    imposto = documento['cteProc']['CTe']['infCte']['imp']['ICMS']
    ...
    cte = {
        ...
        'Chave de Acesso': chave,
        'CFOP': cfop,
        'Município de Origem': municipio_inicio,
        'UF de Origem': uf_inicio,
        'Município de Destino': municipio_final,
        'UF de Destino': uf_final,
        'CNPJ transportadora': cnpj_transportadora,
        'IE transportadora': ie_transportadora,
        'CNPJ/CPF do Remetente': cnpj_remetente,
        'IE Remetente': ie_remetente,
        'CNPJ/CPF do destinatario': cnpj_destinatario,
        'IE Destinatario': ie_destinatario,
